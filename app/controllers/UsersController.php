...
class UsersController extends SecureController{
	...
	function add($formdata = null){
		if($formdata){
			...
			$db->where("uusername", $modeldata['uusername']);
			if($db->has($tablename)){
				$this->view->page_error[] = $modeldata['uusername']." Already exist!";
			}
			//Check if Duplicate Record Already Exit In The Database
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","uusername","role");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'uusername' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			//Check if Duplicate Record Already Exit In The Database
			if(isset($modeldata['uusername'])){
				$db->where("uusername", $modeldata['uusername'])->where("id", $rec_id, "!=");
				if($db->has($tablename)){
					$this->view->page_error[] = $modeldata['uusername']." Already exist!";
				}
			} 
